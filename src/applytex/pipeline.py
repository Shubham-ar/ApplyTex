"""ApplyTex Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    applytex run                        # all stages, sequential
    applytex run --stream               # all stages, concurrent
    applytex run discover enrich        # specific stages
    applytex run score tailor cover     # LLM-only stages
    applytex run --dry-run              # preview without executing
"""
...
from applytex.config import load_env, ensure_dirs, is_latex_enabled, load_app_config, pipeline_min_score
from applytex.database import init_db, get_connection, get_stats
...
LATEX_STAGE_ORDER = ("discover", "enrich", "score", "latex")
CANONICAL_ORDER = ("discover", "enrich", "score", "tailor", "latex", "cover", "pdf")

STAGE_META: dict[str, dict] = {
    "discover": {"desc": "Job discovery (JobSpy + Workday + smart extract)"},
    "enrich":   {"desc": "Detail enrichment (full descriptions + apply URLs)"},
    "score":    {"desc": "LLM scoring (fit 1-10)"},
    "tailor":   {"desc": "Resume tailoring (legacy plain-text LLM)"},
    "latex":    {"desc": "Keyword Match LaTeX tailor + compile"},
...
# it has no remaining pending work. Per-run graph built by _build_upstream().
_LEGACY_UPSTREAM: dict[str, str | None] = {
    "discover": None,
    "enrich":   "discover",
    "score":    "enrich",
    "tailor":   "score",
...
def _pick_upstream(stage: str, candidates: list[str], ordered_set: set[str]) -> str | None:
...
def _build_upstream(ordered: list[str]) -> dict[str, str | None]:
    """Build per-run upstream map from the stages actually being executed."""
    ordered_set = set(ordered)
    upstream: dict[str, str | None] = {}

    for stage in CANONICAL_ORDER:
        if stage not in ordered_set:
            continue
        if stage == "discover":
            upstream[stage] = None
        elif stage == "enrich":
            upstream[stage] = _pick_upstream(stage, ["discover"], ordered_set)
        elif stage == "score":
            upstream[stage] = _pick_upstream(stage, ["enrich", "discover"], ordered_set)
        elif stage in ("tailor", "latex"):
            upstream[stage] = _pick_upstream(stage, ["score", "enrich", "discover"], ordered_set)
        elif stage == "cover":
            upstream[stage] = _pick_upstream(stage, ["latex", "tailor", "score"], ordered_set)
        elif stage == "pdf":
            upstream[stage] = _pick_upstream(stage, ["cover", "latex", "tailor"], ordered_set)
        else:
            upstream[stage] = _LEGACY_UPSTREAM.get(stage)
...
def _run_discover(workers: int = 1) -> dict:
    ...
    try:
        from applytex.discovery.jobspy import run_discovery
        run_discovery()
        stats["jobspy"] = "ok"
    ...
    try:
        from applytex.discovery.workday import run_workday_discovery
    ...
    try:
        from applytex.discovery.smartextract import run_smart_extract
...
def _run_enrich(workers: int = 1) -> dict:
    ...
    try:
        from applytex.enrichment.detail import run_enrichment
...
def _run_score(workers: int = 1) -> dict:
    ...
    try:
        from applytex.scoring.scorer import run_scoring
...
def _run_tailor(min_score: int | None = None, validation_mode: str = "normal") -> dict:
    """Stage: Resume tailoring — generate tailored resumes for high-fit jobs."""
    if min_score is None:
        min_score = pipeline_min_score()
    try:
        from applytex.scoring.tailor import run_tailoring
...
def _run_cover(min_score: int | None = None, validation_mode: str = "normal") -> dict:
    """Stage: Cover letter generation."""
    if min_score is None:
        min_score = pipeline_min_score()
    try:
        from applytex.scoring.cover_letter import run_cover_letters
...
def _run_pdf() -> dict:
    ...
    try:
        from applytex.scoring.pdf import batch_convert
        converted = batch_convert()
        return {"status": "ok", "converted": converted}
    except Exception as e:
        log.error("PDF conversion failed: %s", e)
        return {"status": f"error: {e}", "converted": 0}


def _run_latex(min_score: int | None = None, workers: int = 1) -> dict:
    """Stage: LaTeX Keyword Match — patch master.tex per job and compile PDF."""
    if min_score is None:
        min_score = pipeline_min_score()
    try:
        from applytex.latex.tailor import run_latex_tailoring
        return run_latex_tailoring(min_score=min_score, workers=workers)
    except Exception as e:
        log.error("LaTeX tailoring failed: %s", e)
        return {"status": f"error: {e}"}
...
def _default_stages() -> list[str]:
    """Pipeline stages for 'all' based on config.yaml and LaTeX mode."""
    cfg = load_app_config()
    configured = cfg.get("pipeline", {}).get("default_stages")
    if configured:
        stages = [s for s in configured if s in STAGE_META]
        if is_latex_enabled():
            stages = [s for s in stages if s != "tailor"]
        else:
            stages = [s for s in stages if s != "latex"]
        if stages:
            return stages

    if is_latex_enabled():
        stages = ["discover", "enrich", "score", "latex"]
        if cfg.get("cover", {}).get("enabled", False):
            stages.append("cover")
        return stages
    ...
    for name in stage_names:
        if name not in STAGE_META:
            console.print(
                ...
                f"Available: {', '.join(CANONICAL_ORDER)}, all"
            )
            raise SystemExit(1)
        if name not in resolved:
            resolved.append(name)

    return [s for s in CANONICAL_ORDER if s in resolved]
...
class _StageTracker:
    ...
    def __init__(self):
        self._events: dict[str, threading.Event] = {
            stage: threading.Event() for stage in CANONICAL_ORDER
        }
...
_PENDING_SQL: dict[str, str] = {
    ...
    "latex": (
        "SELECT COUNT(*) FROM jobs WHERE fit_score >= ? "
        "AND full_description IS NOT NULL "
        "AND keyword_report_path IS NULL "
        "AND COALESCE(tailor_attempts, 0) < 5"
    ),
    "cover": (
...
def _count_pending(stage: str, min_score: int | None = None) -> int:
...
def _run_stage_streaming(
    ...
    kwargs: dict = {}
    if stage in ("tailor", "cover", "latex"):
        kwargs["min_score"] = min_score
    if stage in ("tailor", "cover"):
        kwargs["validation_mode"] = validation_mode
    if stage in ("discover", "enrich", "score", "latex"):
        kwargs["workers"] = workers

    upstream = upstream_map.get(stage)
    ...
    while not stop_event.is_set():
        ...
        if pending > 0:
            try:
                result = runner(**kwargs)
                passes += 1
                if stage == "pdf" and isinstance(result, dict) and result.get("converted", 0) == 0:
                    log.warning(
                        "PDF stage: %d pending but nothing converted — stopping to avoid idle loop",
                        pending,
                    )
                    break
            except Exception as e:
...
def _run_sequential(ordered: list[str], min_score: int, workers: int = 1,
    ...
    for name in ordered:
        ...
        try:
            kwargs: dict = {}
            if name in ("tailor", "cover", "latex"):
                kwargs["min_score"] = min_score
            if name in ("tailor", "cover"):
                kwargs["validation_mode"] = validation_mode
            if name in ("discover", "enrich", "score", "latex"):
                kwargs["workers"] = workers
            result = runner(**kwargs)
...
def _run_streaming(ordered: list[str], min_score: int, workers: int = 1,
    ...
    stop_event = threading.Event()
    pipeline_start = time.time()
    upstream_map = _build_upstream(ordered)
    ordered_set = set(ordered)
    ...
    for name in ordered:
        ...
        t = threading.Thread(
            ...
            args=(name, tracker, stop_event, upstream_map, min_score, workers, validation_mode),
            name=f"stage-{name}",
            daemon=True,
        )
        threads[name] = t
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]ApplyTex Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
