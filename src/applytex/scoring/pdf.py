...
def count_pending_pdf() -> int:
    """Count tailored .txt files missing a sibling PDF (legacy pdf stage only).

    Excludes LaTeX-compiled jobs and auxiliary _JOB.txt files.
    """
    ...
    rows = conn.execute(
        ...
        WHERE tailored_resume_path IS NOT NULL
          ...
          AND tailored_latex_path IS NULL
        """
    ).fetchall()
    ...
    for row in rows:
        ...
        if not txt_path.exists():
            continue
        if txt_path.name.endswith("_JOB.txt"):
            continue
        if not txt_path.with_suffix(".pdf").exists():
            pending += 1
    return pending
...
def batch_convert(limit: int = 0) -> int:
    ...
    Args:
        limit: Maximum number of files to convert (0 = no limit).
