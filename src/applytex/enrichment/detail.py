...
from applytex.config import DB_PATH
from applytex.database import get_connection, init_db, ensure_columns
...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from applytex.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from applytex.config import load_base_urls
...
def extract_with_llm(page, url: str) -> dict:
    ...
    try:
        ...
        from applytex.discovery.smartextract import extract_json
...
def run_enrichment(limit: int = 0, workers: int = 1) -> dict:
    ...
    Args:
        limit: Maximum number of jobs per site to process (0 = no limit).
