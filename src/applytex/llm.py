"""
Unified LLM client for ApplyTex.
...
def _detect_provider() -> tuple[str, str, str, str]:
    """Return (base_url, model, api_key, provider) based on environment variables.

    provider is one of: gemini, openai, anthropic, local
    """
    ...
    if anthropic_key or os.environ.get("ANTHROPIC_AUTH_TOKEN"):
        api_key = anthropic_key or os.environ.get("ANTHROPIC_AUTH_TOKEN", "")
        ...
        return (base, model_override or default_model, api_key, "anthropic")
...
class LLMClient:
    ...
    def __init__(self, base_url: str, model: str, api_key: str, provider: str = "openai") -> None:
...
def get_client() -> LLMClient:
    ...
    if _instance is None:
        ...
        log.info("LLM provider: %s  model: %s  endpoint: %s", provider, model, base_url)
        _instance = LLMClient(base_url, model, api_key, provider)
    return _instance
