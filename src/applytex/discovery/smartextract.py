...
from applytex.config import CONFIG_DIR
from applytex.database import get_connection, init_db, store_jobs, get_stats
...
def _load_location_filter(search_cfg: dict | None = None):
    ...
    return load_location_patterns(search_cfg)
...
def _store_jobs_filtered(
    ...
    for job in jobs:
        ...
        if not url:
            continue
        if title_excluded(job.get("title"), exclude_titles or []):
            filtered += 1
            continue
    ...
    if filtered:
        log.info("Filtered %d jobs (location/title)", filtered)
    conn.commit()
    return new, existing
...
def build_scrape_targets(
    ...
    max_tier = resolve_max_query_tier(search_cfg)
    queries_cfg = filter_queries_by_tier(queries_cfg, max_tier)
    queries = [q["query"] for q in queries_cfg]
    scrape_locations = resolve_scrape_locations(search_cfg)
    uses_location = "{location_encoded}" in "".join(
        site.get("url", "") for site in sites if site.get("type") == "search"
    )
    location_variants = scrape_locations if uses_location else [scrape_locations[0] if scrape_locations else ""]
    ...
    for site in sites:
        ...
        if site_type == "search" and queries:
            ...
            default_location = scrape_locations[0] if scrape_locations else ""
            expanded_url = site_url
...
def run_smart_extract(
    ...
    return _run_all(targets, accept_locs, reject_locs, exclude_titles=exclude_titles, workers=workers)
