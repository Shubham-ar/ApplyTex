...
parses salary ranges, and stores everything in the ApplyTex database.
...
from applytex.database import get_connection, init_db, store_jobs
...
def _run_one_search(
    ...
    if exclude_titles:
        ...
        df = df[df.apply(
            lambda row: not title_excluded(
                str(row.get("title", "")) if str(row.get("title", "")) != "nan" else None,
                exclude_titles,
            ),
            axis=1,
        )]
        title_filtered = before_titles - len(df)
        if title_filtered:
...
def _full_crawl(
    ...
    search_cfg = normalize_search_config(search_cfg)
    if sites is None:
        sites = search_cfg.get("sites") or ["indeed", "linkedin", "zip_recruiter"]
    ...
    queries = filter_queries_by_tier(queries, max_tier)
    ...
    if locations:
        locs = [
            loc for loc in locs
            if loc.get("label") in locations or loc.get("location") in locations
        ]
    ...
    for q in queries:
        for loc in locs:
            if isinstance(loc, str):
                loc_entry = {"location": loc, "remote": False}
            elif isinstance(loc, dict) and loc.get("location"):
                loc_entry = loc
            else:
                log.warning("Skipping invalid locations entry: %r", loc)
                continue
            searches.append({
                "query": q["query"],
                "location": loc_entry["location"],
                "remote": loc_entry.get("remote", False),
                "tier": q.get("tier", 0),
            })
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if cfg is None:
        ...
        cfg = normalize_search_config(cfg)

    if not cfg:
        log.warning("No search configuration found. Run `applytex init` to create one.")
